...
def main(json_path='options/train_mbsr.json'):
    ...
    # ----------------------------------------
    logger_name = 'mytrain'
    utils_logger.logger_info(logger_name, os.path.join("results", logger_name+'.log'))
    ...
    for epoch in range(opt['train']['n_epochs']):  # keep running
        for i, train_data in enumerate(tqdm(train_loader)):
            ...
            # -------------------------------
            # 1) update learning rate new version of pytorch requires schedule after optimizer
            # -------------------------------
            ...
            if current_step % 1 == 0:
                ...
                for test_data in test_loader:
                    ...
                    # calculate PSNR
                    # -----------------------
                    current_psnr = util.calculate_psnr(E_img[:,:,:3], H_img[:,:,:3], border=border)
